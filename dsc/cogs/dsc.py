import asyncio, aiohttp, discord, json, logging, random
...
class SearchResultsView(View):
    def __init__(self, results: List[Tuple[int, str, str, str]], ctx: commands.Context, footer_text: str, timeout: float = 60.0):
        ...
        self.results, self.ctx, self.footer_text = results, ctx, footer_text
        self.results_per_page, self.page = 5, 1
        self.total_pages = (len(results) - 1) // 5 + 1
...
class DscCog(commands.Cog):
    def __init__(self, bot):
        self.bot, self.session = bot, aiohttp.ClientSession()
    ...
    async def user_settings(self, ctx, value: str):
    ...
    async def send_random_page(self, ctx):
        Settings.get_user_setting(str(ctx.author.id))
    ...
    async def search_with_tags(self, ctx, *tags):
        ...
        for a in soup.select("#tagged-pages-list a"):
            ...
            text = await self.fetch_article_text(href)
            soup_tags = BeautifulSoup(await self.scraper.fetch_html(href, self.session), "lxml")
    ...
    async def search_name(self, ctx, *, pagename):
        Settings.get_user_setting(str(ctx.author.id))
        links = await self.scraper.get_all_article_links(self.session)
        titles = [(t, u) for t, u in links if pagename.lower() in t.lower()]
        if not titles: return await ctx.send(f"Страница '{pagename}' не найдена.")
        t, u = titles[0]
        text = await self.fetch_article_text(u)
        sent = text.split(".")[0].strip() + "." if text else "Содержимое не найдено."
    ...
    async def search_excerpt(self, ctx, *, query):
        Settings.get_user_setting(str(ctx.author.id))
        articles = await self.scraper.get_all_article_links(self.session)
        results = []
        for title, url in articles:
            if "draft:" in url: continue
            text = await self.fetch_article_text(url)
            if query.lower() in text.lower():
                snippet = TextProcessing.extract_sentence(text, query) or TextProcessing.trim_text(text, 300)
